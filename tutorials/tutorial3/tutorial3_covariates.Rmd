...
settlement type. We integrate those large-scale variations in a Bayesian
framework by using a hierarchical random intercept model.
...
great to inform about the local context:
...
typically spatial covariates with national coverage and consistent data
collection. While individual or household-level information, such as
collected during surveys, is useful for understanding differences in
...
Because the mean of the population density is only defined with a random
intercept, it results in 5 (number of settlement type) x 11 (number of
region) options for the population density estimates. To add small-scale
variations we refine the mean with a regression model that integrates
the covariates.

More formally, let's define $X$ a matrix of size the number of
observation x number of covariates that contains the covariates values
for each cluster and $\beta$ a vector of size the number of covariates.
Based on Equation 3 in tutorial 2 (and removing the prior definition of
$\alpha_{t,r}$ for sake of readibility), we define $\mu$ the mean of the
...
review_cov %>% arrange(Type) %>% kbl(caption='Review of covariates used in WorldPop bottom-up population estimates') %>% kable_minimal()
```
...
2.  *infrastructure* locations through OpenStreetMap
...
4.  *settlement shape* through morphology of building fooptrints
...
our specific population data. It can reach up to 100+. We then use
...
To select the best one for prediction purposes, we use two methods:
...
Six covariates were used in Nigeria v1.2 model [@leasure2020]:
...
pixels within each microcensus cluster. They treated this covariate as
an indicator of relative population densities based on geospatial
...
the average of the covariate values for each cluster using zonal
statistics.

Note that this constitutes a change in support: we might want to check
if the range of covariates values at cluster level is representative of
the covariate values at grid cell level.

Figure \@ref(fig:scatter) shows the relation between the covariates and
the population density at cluster level. We see that household size
...
```{r scatter, message=F, fig.cap='Scatterplot of covariates values vs population density for each cluster', class.source = 'fold-hide'}
# prepare data
...
covariates at cluster level**, such that the $\beta_k$ have the same
...
We store the scaling coefficients for the prediction stage.

```{r}
write_csv(scale_factor, 'tutorials/tutorial3/scale_factor.csv')
```
...
inclusion of covariates: in a for loop that runs through each cluster
observations, such that `cov[idx,]` is a (column)-vector and requires a
`row_vector` as multiplier.

We keep the same set up for `stan` Markov chains:
...
We can then compute the predictions for every cluster and compare the
...
```{r scatter-t, class.source = 'fold-hide', fig.cap='Scatterplot of covariates vs population density by settlement type'}
ggplot(data_long %>% 
         group_by(type) %>% 
         mutate(
...
covariates that are fixed and the one that is random.
