...
(`x_3`) is positively assocaited with population density. The
negative value are due to the scaling method adopted. On the opposite,
nonresidential settled area (`x_6`) is negatively associated which is
...
simulations that are the backbone of Bayesian estimation needs to be
given an initial value to start exploring the parameter space.

It is in `stan` controlled by the option `init` whose default is
...
Based on our understanding of population distribution,
we define the start of the estimation process in the environs of the likely parameters value.

We will initialise only root parameters, that are not depending on other parameters.
```{r}
...
We only observe the same issue than in tutorial 2 that is integrer overflow for the prediction of population in the warmup period.
