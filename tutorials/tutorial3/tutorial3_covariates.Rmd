...
simulations,backbone of Bayesian estimation, need to start exploring 
the parameter space at one value.

This starting value is in `stan` controlled by the option `init` whose default is
...
Defining the starting values means helping the algorithm to start close to the region of interest
...
We define the start of the estimation process based on our understanding of population distribution.
let's state that initialisation is not constraining, it just gives a hint to the algorithm. 
