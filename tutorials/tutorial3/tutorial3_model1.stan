...
// Model 5: Hierarchical alpha by settlement type , region, state local + cauchy prior + vectorisation
...
transformed parameters{
  ...
  for(idx in 1:n){
    pop_density_mean[idx] = alpha_t_r_s_l[type[idx], region[idx], state[idx], local[idx]] + sum( cov[idx,] .* beta );
  }
...
generated quantities{
  ...
  for(idx in 1:n){
    density_hat[idx] = lognormal_rng( alpha_t_r_s_l[type[idx], region[idx], state[idx], local[idx]] + sum(cov[idx,] .* beta), sigma );
