...
-   [Markov chain Monte Carlo
    ...
    -   [Markov chains explained
        visuall](https://setosa.io/ev/markov-chains/)
    -   [Metropolis-Hastings Monte
        Carlo](https://www.bayesrulesbook.com/chapter-7.html)
...
Note: the `stan` code for this model mixes two approaches: vectorized
(in the model block) and unvectorized, that is based on a for loop (in
the generated quantities block). This is due to the format of
`normal_rng` draws that are scalars. For more details, please have a
look at
[here](https://mc-stan.org/docs/2_27/stan-users-guide/linear-regression.html#vectorization.section)
and
[here](https://mc-stan.org/docs/2_27/functions-reference/vectorization.html)
in the `stan` manual.
...
likely to overcome this upper bound during the warm-up period when
...
for every cluster. The predicted `population_count` is in contrast is
...
And for that purpose we will stoe this last model:
```{r, eval=F}
...
knitr::purl("tutorial1_linearRegression.Rmd", documentation = 0)
```
