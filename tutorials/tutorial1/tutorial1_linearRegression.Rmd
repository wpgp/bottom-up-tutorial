...
The second part will be devoted to presenting the dataset we will be
...
Now we want to model $y$, our observations. We see that $y$ has a clear
...
The data consists of household surveys that collected information on the
...
mean of the Poisson distribution. We will choose relatively uninformed
priors based on our understanding of the data.
...
two parameters --- mean and standard deviation --- and the site-level
...
The left hand side of the equation is a discrete observed variable
whereas the right hand side is composed of two continuous variables, one
observed, the *settled_area* and one latent, the *pop_density*.
...
*pop_density* that can be modelled with its own distribution. We opt for
...
distribution for every survey sites. The predicted `population_count`
is, in contrast, influenced by the `settled_area` and by the variance
term that adapts the confidence intervals for each cluster.
...
Douglas Leasure from Leverhulme Centre for Demographic Science,
...
4.0]([https://creativecommons.org/licenses/by-nd/4.0/))](https://creativecommons.org/licenses/by-nd/4.0/)))
license.
