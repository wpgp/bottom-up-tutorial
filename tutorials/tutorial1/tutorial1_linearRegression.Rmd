...
subtitle: "Tutorial 1: How to think population as a Bayesian?"
author: ""
...
We produce our fake data as 1000 draws of a Normal with mean 5 and
...
bell shape. It can thus be **approximate with a Normal distribution**
that is: $$Y 〜 Normal(mean=\mu, sd=\sigma)$$
...
Figure \@ref(fig:prior-mu-distrib) shows what does our prior for $\mu$
means: we think that $\mu$ is likely to be around zero but could be up
to 200. Given that our observed $y$, $\hat{y}$, has no occurrences above
200, it is unlikely that the mean of $y$ is 200 but we don't exclude the
...
Y 〜 Normal( \mu, \sigma ) \\[1pt]
\\
\mu 〜 Normal( 0, 100 ) \\
\sigma 〜 Uniform( 0, 200 )(\#eq:modeltheo)
...
support, e.g. it is not possible to have a negative $\sigma$
(`real<lower=0> sigma`).
...
We see that (1) the observed mean and standard deviation are within the 95% credible intervals of the estimated parameters, 
(2) the true mean and standard deviation are within the 95% credible intervals of the estimated parameters,
...
population 〜 Poisson( \lambda ) \\
\\
\lambda 〜 Uniform( 0, 3000 )(\#eq:model1)
...
the uniform prior for $\lambda$ declaring than any value between 0 and
...
The estimated mean correspond to the observed mean.
...
We note that all predictions are very similar. The moodel includes only two parameters 
— mean and standard deviation — and the site-level predictions do not account for any site-level characteristics. 
Therefore, predictions at each site are drawn from the exact same distribution.
...
subnational variations introduced. Furthermore the credible intervals
...
different sizes of settled areas as shown in Figure \@ref(fig:popvsarea)

```{r popvsarea,  class.source = 'fold-hide', caption='Scatterplot contrsating population count with settled area for every cluster'}
...
This formulation unravels a continuous positive latent variable,
...
log(pop\_density)  〜 Normal( \mu, \sigma)
...
pop\_density  〜 exp(Normal( \mu, \sigma))

\end{equation}
```
### Defining the priors

In the Lognormal, there is two parameters, $\mu$ that represents the
...
stan_data_model2 <- list(
  ...
  area = data$A)
```
Note that the population density is not an input data of the model, but a unobserved latent variable that we model 
...
The proportion of observations that falls in their credible intervals
shows a well-behave model: 95.5% of the observations falls in the 95%
credible interval. In average the model overestimates the population
count by around 60 people, but this statistics varies in average by 337
people.

No variations is included in this model: we build a national model of
