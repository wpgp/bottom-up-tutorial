...
on the estimated parameters with a *posterior predictive check*, on the
spatial autocorrelation of the residuals with a *spatial correlogram*
...
`alphas` contains the estimated alpha for each iteration post-warmup
...
The coordinates of the study sites were not communicated with the data
for confidentiality reason so we will reproduce the correlogram from
...
observations for model fitting and leaving the remaing study sites for
...
\@ref(eq:predict) with $\hat\mu$ and $\hat\sigma$.
...
`predictions` contains 2000 population estimates for every settled grid
cell.
...
Bayesian modelling is a stochastic modelling thus accounts for the
...
Figure \@ref(fig:raster-mean) shows typical population distribution:
denser around cities and following the roads as seens by the lines of
higher population count. In white are displayed the unsettled grid
cells.
...
We see that higher uncertainty can be observed on the outskirt of urban
areas. This is often the case due to the rapid expansion of settlement
