...
subtitle: "Tutorial 2: How to represent spatial variations \nwith a hierarchichal structure?"
...
-   [Hierarchical models
    visualised](http://mfviz.com/hierarchical-models/) by Michael
    Freeman

-   [Bayes rules Book on hierarchical
    model](https://www.bayesrulesbook.com/chapter-15.html) by Alicia A.
    Johnson, Miles Ott, Mine Dogucu: good introduction to hierarchically
...
evaluating Bayesian estimations.
...
across Nigeria. We can already observed clustered patterns with some
regions that have a higher population density in average.

```{r pop-map, echo=F,fig.cap='Map of cluster population densities overlaid with Nigeria region boundaries', purl=F}
...
clusters. The underlying assumption: the administrative structure of a
country gives information on the population density variation.
...
We see that settlement type clearly stratifies the clusters.

Note that out of the six settlement types displayed in Figure
\@ref(fig:sett-map), only five types are actually present in the data.
No clusters were surveyed in the non-residential type area.

## Full picture of the data grouping

Figure \@ref(fig:group-schema) shows the full structure of the surveyed
clusters, with the following nesting: settlement type, region, state and
local government area. When hovering on the schema, you can see how many
survey clusters are in each grouping. You can access the detailed
...
2.  not every group combination is represented in the data. Some
    region/state/local governmental area do not have clusters belonging
    to all settlement types. Figure \@ref(fig:group-complete-schema)
    shows the regions with missing settlement type. We can't assess from
    this analysis if those settlement types are missing because they
    have not being sampled in our survey or they don't exist (for
    example a remote region that does not contain any highly urban
    settlement type).
...
the population density distribution (c.f. the moment of order 1 of a
...
Let's see the different scenarios for setting up $\alpha$ (we will show
only for three settlement types for sake of simplicity):
...
Tutorial 1 build a model based on complete pooling.
...
We first declare the observation structure of settlement type.

Then $\alpha$ becomes a vector of size 5, the number of settlement type
in the data.

We choose the same prior for every $\alpha_t$, five normal distribution
centered on zero and with standard deviation 100.

Notice how the indexing works in the *generated quantities* block.

Once the model is written, we prepare the corresponding data, that is
adding the settlement type:
...
And run the model is a similar fashion as in tutorial 1:
...
```{r model1-alpha-t, message=F, warning=F, fig.cap='Intercept estimated independantly by settlement type'}
...
We can draw the observed vs predicted plot to see the impact of
estimating $\alpha$ by settlement type:

```{r, class.source = 'fold-hide', message=F, fig.cap='Comparison of observations with predictions from model 1'}
...
We know see that the predicted population density has five modes that
...
In other words $\alpha$ define athe country-wide distribution that
constrain each $\alpha_t$ to be in the same range.
...
not strikingly, which indicates that the number of clusters per
...
uncertainty (see *Mean Confidence Interval Size*).
...
\alpha 〜 Normal( 0, 100 ) \\
\nu_\alpha_t \sim Uniform( 0, 100 ) \\
\nu_\alpha 〜 Uniform( 0, 100 ) \\
\sigma 〜 Uniform( 0, 100 )(\#eq:model3)

\end{equation} \$\$

Which would be translated in `stan` as :
...
```{r}
pars <- c('alpha','alpha_t','alpha_t_r', 'sigma','population_hat',  'density_hat')
...
stan_plot(fit3, pars=c('alpha','alpha_t[1]', alpha_1_r,'alpha_t[4]', alpha_4_r), fill_color='orange')

```

We see that:

1.  $\hat\alpha$ has a estimated distribution that overlap the two
...
3.  those settlement-level patterns masks some regional disparities
...
that their estimated means closely match their respective
$\hat\alpha_t$.
...
very different characteristics, the model will not be able to estimate
them

## A hierarchical intercept per settlement type, region, state and local government area
...
The additional administrative level howvere are organised in a *nested
...
```{r, message=F}
data %>% group_by(region, state) %>% summarise(`Number of clusters`=n()) %>% 
  kbl() %>% kable_minimal()
```
...
inits <- function(md, nchains=4){
  ...
  for (c in 1:nchains){
    inits.i <- list()
    # intercept
    inits.i$alpha_t_r_s_l <- array(runif(md$ntype * md$nregion * max(md$nstate) * md$max_nlocal, 0, 2.5),
                            dim=c(md$ntype, md$nregion, max(md$nstate), md$max_nlocal))
    inits.i$alpha <- runif(1, 0, 2.5)
    inits.i$alpha_t <- runif(md$ntype, 0, 2.5)
    inits.i$alpha_t_r <- matrix(runif(md$ntype * md$nregion, 4, 8), nrow=md$ntype, ncol=md$nregion)
    inits.i$alpha_t_r_s <- array(runif(md$ntype * md$nregion * max(md$nstate), 0, 2.5), 
...
```{r, eval=F}
pars <- c('alpha','alpha_t','alpha_t_r','alpha_t_r_s','alpha_t_r_s_l', 'sigma','population_hat',  'density_hat')
