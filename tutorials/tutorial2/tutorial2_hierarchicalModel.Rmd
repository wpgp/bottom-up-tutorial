...
stan restricts the value for the Poisson rate to 1.07374e+09. We reached
this value however in our estimation process due the parameter space
exploration.

To correct for it, we just change the scale of our data by multiplying
the area by 1000 such that log of the observed population density has
now for maximum 0.01.

```{r}
# prepare data for stan
stan_data_model3bis <- list(
  population = data$N,
  n = nrow(data),
  area = data$A*1000,
  type = data$type,
...
We account for this change of scale in the prior definition of $\alpha$,
from a Normal(5,10) to a Normal(0,10) and store the model under
`tutorial2_model3bis.stan`.

```{r}
# mcmc
fit3bis <- rstan::stan(file = file.path('tutorial2_model3bis.stan'), 
                   data = stan_data_model3bis,
                   iter = warmup + iter, 
                   chains = chains,
                   warmup = warmup, 
                   pars = pars,
...
We don't observe any problem of convergence or estimation anymore.
...
Bear in mind that we are looking at the median of the lognormal for a
rescaled population density. As a benchmark the observed rescaled median
population density on the log scale is: \`r
round(median(log(summary(data\$pop_density/1000))),2)\`
...
stan_plot(fit3bis, pars=c('alpha','alpha_t[1]','alpha_t[4]', alpha_1_r, alpha_4_r), 
          fill_color='orange')
